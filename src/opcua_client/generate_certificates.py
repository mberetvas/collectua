...
CLIENT_USE = [ExtendedKeyUsageOID.CLIENT_AUTH]
...
def generate_private_key_for_myclient():
    key: RSAPrivateKey = generate_private_key()
    key_file = base_private / "myclient.pem"
    key_file.write_bytes(dump_private_key_as_pem(key))
...
async def generate_self_signed_certificate_for_myclient():
    """Generate a self-signed certificate for OPC UA client application."""
    subject_alt_names: list[x509.GeneralName] = [
        x509.UniformResourceIdentifier(f"urn:{HOSTNAME}:foobar:myclient"),
        x509.DNSName(f"{HOSTNAME}"),
    ]

    key = await load_private_key(base_private / "myclient.pem")

    cert: x509.Certificate = generate_self_signed_app_certificate(
        key, f"myclient@{HOSTNAME}", NAMES, subject_alt_names, extended=CLIENT_USE
    )

    cert_file = base_certs / "myclient-selfsigned.der"
    cert_file.write_bytes(cert.public_bytes(encoding=Encoding.DER))
...
async def main():
    ...
    generate_private_key_for_myclient()
    await generate_self_signed_certificate_for_myclient()
